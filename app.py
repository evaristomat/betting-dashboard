...
def get_apostas_do_dia():
    """
    Carrega e filtra as apostas do dia atual, selecionando as 2 melhores de cada jogo
    """
    try:
        # Carrega todas as apostas pendentes
        df_pending = pd.read_csv("bets/bets_atualizadas.csv")

        # Filtra apenas apostas pendentes
        df_pending = df_pending[df_pending["status"] == "pending"].copy()
        ...
        # Cria identificador único do jogo
        df_hoje["jogo_id"] = (
            df_hoje["t1"].astype(str) + " vs " + df_hoje["t2"].astype(str)
        )

        # Calcula score de qualidade da aposta
        df_hoje["ROI_num"] = pd.to_numeric(df_hoje["ROI"], errors="coerce")
        df_hoje["odds_num"] = pd.to_numeric(df_hoje["odds"], errors="coerce")

        # Score baseado em ROI e odds balanceadas
        df_hoje["score_aposta"] = (
            df_hoje["ROI_num"] * 0.6  # ROI tem maior peso
            + (df_hoje["odds_num"] * 10) * 0.4  # Odds multiplicadas para normalizar
        )

        # Remove linhas com score inválido
        df_hoje = df_hoje.dropna(subset=["score_aposta"])

        if len(df_hoje) == 0:
            return pd.DataFrame()

        # Seleciona as 2 melhores apostas por jogo
        melhores_apostas = (
            df_hoje.groupby("jogo_id")
            .apply(lambda x: x.nlargest(2, "score_aposta"))
            .reset_index(drop=True)
        )

        return melhores_apostas
...
def criar_card_aposta_visual(aposta_data, index):
    """
    Cria um card visual para cada aposta usando HTML/CSS
    """
    # Cores baseadas no ROI
    roi_value = float(aposta_data.get("ROI_num", 0))
    if roi_value >= 15:
        cor_roi = "#00C851"  # Verde forte
        emoji_roi = "🔥"
    elif roi_value >= 10:
        cor_roi = "#2E7D32"  # Verde médio
        emoji_roi = "⭐"
    elif roi_value >= 5:
        cor_roi = "#FFB74D"  # Laranja
        emoji_roi = "⚡"
    else:
        cor_roi = "#757575"  # Cinza
        emoji_roi = "📊"
    ...
    ">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
            <div style="flex: 1;">
                <h3 style="color: white; margin: 0 0 8px 0; font-size: 18px; font-weight: bold;">
                    {emoji_roi} {aposta_data.get("t1", "Team 1")} vs {aposta_data.get("t2", "Team 2")}
                </h3>
                <p style="color: #B0B0B0; margin: 0; font-size: 14px;">
                    🏆 {aposta_data.get("league", "League")} • 📅 {aposta_data.get("date", "").strftime("%H:%M") if pd.notna(aposta_data.get("date")) else "N/A"}
                </p>
            </div>
            <div style="
                background: {cor_roi};
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-bottom: 15px;">
            <div style="text-align: center;">
                <p style="color: #B0B0B0; margin: 0; font-size: 12px;">APOSTA</p>
                <p style="color: white; margin: 5px 0 0 0; font-weight: bold; font-size: 14px;">
                    {aposta_data.get("bet_type", "N/A")}
                </p>
            </div>
            <div style="text-align: center;">
                <p style="color: #B0B0B0; margin: 0; font-size: 12px;">LINHA</p>
                <p style="color: white; margin: 5px 0 0 0; font-weight: bold; font-size: 14px;">
                    {aposta_data.get("bet_line", "N/A")}
                </p>
            </div>
            <div style="text-align: center;">
                <p style="color: #B0B0B0; margin: 0; font-size: 12px;">ODD</p>
                <p style="color: {cor_roi}; margin: 5px 0 0 0; font-weight: bold; font-size: 16px;">
                    {float(aposta_data.get("odds_num", 0)):.2f}
                </p>
            </div>
        </div>
        ...
        <div style="
            ...
            text-align: center;
        ">
            <p style="color: #B0B0B0; margin: 0; font-size: 12px;">CASA DE APOSTAS</p>
            <p style="color: white; margin: 5px 0 0 0; font-weight: bold;">
                {aposta_data.get("House", "N/A")}
            </p>
        </div>
    </div>
    """
...
def criar_grafico_roi_distribuicao(df_apostas):
    """
    Cria gráfico de pizza mostrando distribuição dos ROIs
    """
    if len(df_apostas) == 0:
        return None
    ...
    def categorizar_roi(roi):
        if roi >= 15:
            return "ROI Excelente (15%+)"
        elif roi >= 10:
            return "ROI Muito Bom (10-14%)"
        elif roi >= 5:
            return "ROI Bom (5-9%)"
        else:
            return "ROI Moderado (<5%)"

    df_apostas["categoria_roi"] = df_apostas["ROI_num"].apply(categorizar_roi)

    # Conta apostas por categoria
    roi_counts = df_apostas["categoria_roi"].value_counts()

    # Cores para cada categoria
    colors = {
        "ROI Excelente (15%+)": "#00C851",
        "ROI Muito Bom (10-14%)": "#2E7D32",
        "ROI Bom (5-9%)": "#FFB74D",
        "ROI Moderado (<5%)": "#757575",
    }
    ...
    fig = go.Figure(
        data=[
            go.Pie(
                ...
                hole=0.4,
                marker_colors=[colors.get(cat, "#757575") for cat in roi_counts.index],
    ...
    fig.update_layout(
        title={
            "text": "Distribuição de ROI das Apostas do Dia",
            "x": 0.5,
...
def display_apostas_do_dia():
    """
    Função principal para exibir a seção Apostas do Dia
    """
    st.markdown("---")
    st.markdown("## 🌟 Apostas do Dia")

    # Carrega apostas do dia
    apostas_hoje = get_apostas_do_dia()

    if len(apostas_hoje) == 0:
        st.info(
            "📭 Nenhuma aposta encontrada para hoje. As apostas aparecem aqui quando há jogos agendados para o dia atual."
        )
        return

    # Header com estatísticas
    total_apostas = len(apostas_hoje)
    total_jogos = apostas_hoje["jogo_id"].nunique()
    roi_medio = apostas_hoje["ROI_num"].mean()
    melhor_roi = apostas_hoje["ROI_num"].max()
    ...
    with col1:
        st.metric(
            ...
            value=f"{total_apostas}",
            help="Melhores apostas selecionadas para hoje",
        )

    with col2:
        st.metric(
            label="🏟️ Jogos do Dia",
            value=f"{total_jogos}",
            help="Número de jogos com apostas disponíveis",
        )
    ...
    with col3:
        st.metric(
            ...
            help="ROI médio das apostas selecionadas",
        )

    with col4:
        st.metric(
            label="🔥 Melhor ROI",
            value=f"{melhor_roi:.1f}%",
            help="Maior ROI encontrado hoje",
        )

    # Gráfico de distribuição ROI
    fig_roi = criar_grafico_roi_distribuicao(apostas_hoje)
    if fig_roi:
        st.plotly_chart(fig_roi, use_container_width=True)

    # Cards das apostas organizados por jogo
    st.markdown("### 🎮 Apostas Recomendadas por Jogo")

    for jogo_id in apostas_hoje["jogo_id"].unique():
        apostas_jogo = apostas_hoje[apostas_hoje["jogo_id"] == jogo_id]

        st.markdown(f"#### {jogo_id}")

        # Cria colunas para as apostas do jogo
        if len(apostas_jogo) == 1:
            # Se só tem 1 aposta, usa coluna única
            st.markdown(
                criar_card_aposta_visual(apostas_jogo.iloc[0]), unsafe_allow_html=True
            )
        else:
            # Se tem 2 apostas, usa 2 colunas
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(
                    criar_card_aposta_visual(apostas_jogo.iloc[0]),
                    unsafe_allow_html=True,
                )
            if len(apostas_jogo) > 1:
                with col2:
                    st.markdown(
                        criar_card_aposta_visual(apostas_jogo.iloc[1]),
                        unsafe_allow_html=True,
                    )

    # Tabela detalhada (opcional, colapsível)
    with st.expander("📋 Ver Detalhes Completos das Apostas"):
        display_cols = [
            col
            for col in [
                "date",
            ...
            if col in apostas_hoje.columns
        ]

        # Renomeia colunas para melhor visualização
        apostas_display = apostas_hoje[display_cols].copy()
        if "score_aposta" in apostas_display.columns:
            apostas_display = apostas_display.rename(
                columns={"score_aposta": "Score Qualidade"}
            )

        st.dataframe(apostas_display, use_container_width=True, hide_index=True)
